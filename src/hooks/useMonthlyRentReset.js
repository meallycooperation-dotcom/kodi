...
const useMonthlyRentReset = () => {
    ...
    const lastCheckedMonth = useRef(null);
    useEffect(() => {
        if (!user?.id)
            return;
        const resetMonthlyRent = async () => {
            try {
                ...
                if (lastCheckedMonth.current === currentMonth)
                    return;
                lastCheckedMonth.current = currentMonth;
                // Get active tenants
                const activeTenants = tenants.filter((tenant) => tenant.status === 'active' || tenant.status === 'late');
                // Check which tenants need a payment entry for this month
                for (const tenant of activeTenants) {
                    if (!tenant.moveInDate || !tenant.unitId)
                        continue;
