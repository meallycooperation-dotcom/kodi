...
const PaymentForm = ({ tenants: tenantOptions, units: unitOptions, apartmentId, apartmentBlockId } = {}) => {
    const { user } = useAuth();
    const { refresh } = usePayments();
    const [units, setUnits] = useState(unitOptions ?? []);
    const [tenants, setTenants] = useState(tenantOptions ?? []);
        ...
        try {
            ...
            if (!apartmentId) {
                ...
                if (typeof window !== 'undefined') {
                    window.dispatchEvent(new CustomEvent('apartment-payment-recorded', {
                        detail: { apartmentId }
                    }));
                }
            }
