...
const MonthlyRevenueChart = ({ data, width = 600, height = 320 }: MonthlyRevenueChartProps) => {
  ...
  useEffect(() => {
    ...
    const xExtent = d3.extent(preparedData, (d) => d.date) as [Date, Date];
    ...
    const maxValue =
      d3.max(preparedData, (row) =>
        Math.max(row.collected_revenue, row.total_arrears, row.expected_revenue)
      ) ?? 0;
    ...
    const lineFactory =
      (key: keyof ChartDatum) =>
      d3
        .line<ChartDatum>()
        .x((datum) => xScale(datum.date))
        .y((datum) => yScale(datum[key] as number));
    ...
    const annotateSeries = (key: keyof ChartDatum, color: string) => {
      group
        .append('g')
        .selectAll('circle')
        .data(preparedData)
        ...
        .attr('cy', (datum) => yScale(datum[key] as number))
        .attr('r', 3.5)
        .attr('fill', color)
        .attr('stroke', '#fff')
        .attr('stroke-width', 1);
  ...
  return (
    <div className="w-full">
      <svg ref={ref} role="img" aria-label="Monthly revenue timeline" />
    </div>
  );
};
